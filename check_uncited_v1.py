...
def main():
    ...
    for ref in parsed_refs:
        ...
        # Generate search pattern for current reference (Author Fingerprint + Year).
        auth1_esc = re.escape(ref['authors'][0])
        year_esc = re.escape(year)
        
        # Match pattern supports variations (e.g. Author et al, 2020 or Author & Author 2020).
        if len(ref['authors']) >= 2:
            auth2_esc = re.escape(ref['authors'][1])
            pattern_str = f"{auth1_esc}.{{0,60}}{auth2_esc}.{{0,60}}{year_esc}"
            pattern_alt = f"{auth1_esc}.{{0,80}}{year_esc}"
            regex = re.compile(f"({pattern_str})|({pattern_alt})", re.IGNORECASE | re.DOTALL)
        else:
            pattern_str = f"{auth1_esc}.{{0,80}}{year_esc}"
